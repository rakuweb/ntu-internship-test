...
import {
  ...
  ContactSchema,
  useContactFormStore,
} from '~/features/contact/coworking';
...
export const Presenter: FC<PresenterProps> = ({ ...props }) => {
  ...
  const submitHandler = async (data: ContactSchema) => {
    ...
    try {
      const _res = await axios.post(routes.apiCoworkingContact, { ...remain });
      reset({
        item: '',
        desired_visit_date: undefined,
        company_name: '',
        manager_name: '',
        manager_phone: '',
        email: '',
      ...
      router.push(routes.coworkingContactComplete);
    } catch (e) {
      console.error(e);
    } finally {
      setIsSending(false);
